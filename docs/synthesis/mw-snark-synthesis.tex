...
development also adds four non-historical stress tests: stable merge, stable
partition, addition simplification, and shortest-path relaxation.  It is not a
verbatim
...
\subsection{New Addition Simplifier}

For an addition node \(e_1+e_2\), fold constants when both arguments are
constants; otherwise eliminate additive zero on either side; otherwise rebuild
from recursively simplified children.  SNARK input:
\begin{lstlisting}
(assert '(implies
...
Answer:
\begin{lstlisting}
(ANSWER-IF (NEW-ZERO E2)
  (ANSWER-IF (NEW-ZERO E1)
    (ANSWER-IF (NEW-BOTH-CONST E1 E2)
      (VALUES (NEW-CONST (NEW-ADD-VALUE E1 E2)))
      (VALUES (NEW-SIMPLIFY E2)))
    (ANSWER-IF (NEW-BOTH-CONST E1 E2)
      (VALUES (NEW-CONST (NEW-ADD-VALUE E1 E2)))
      (VALUES (NEW-SIMPLIFY E1))))
  (ANSWER-IF (NEW-ZERO E1)
    (ANSWER-IF (NEW-BOTH-CONST E1 E2)
      (VALUES (NEW-CONST (NEW-ADD-VALUE E1 E2)))
      (VALUES (NEW-SIMPLIFY E2)))
    (ANSWER-IF (NEW-BOTH-CONST E1 E2)
      (VALUES (NEW-CONST (NEW-ADD-VALUE E1 E2)))
      (VALUES (NEW-PLUS (NEW-SIMPLIFY E1)
                        (NEW-SIMPLIFY E2))))))
\end{lstlisting}

\subsection{New Shortest-Path Relaxation}

For an edge \(u\xrightarrow{w}v\), update the distance and predecessor of
\(v\) exactly when the candidate path through \(u\) improves the current
distance:
\begin{align*}
  d(u)+w<d(v) &\Rightarrow
    s'=\mathrm{pred}(\mathrm{dist}(s,v,d(u)+w),v,u),\\
  \neg(d(u)+w<d(v)) &\Rightarrow s'=s.
\end{align*}
SNARK input:
\begin{lstlisting}
(assert '(implies
...
(ANSWER-IF
  ...
  (VALUES
    (NEW-UPDATE-PREDECESSOR
      (NEW-UPDATE-DISTANCE
        STATE V (NEW-PLUS (NEW-DISTANCE U STATE) W))
      V U))
  (VALUES STATE))
\end{lstlisting}
...
\item the new stable-merge, stable-partition, simplifier, and graph-relaxation
examples use abstract list, expression, and graph-state theories;
...
rewrite predicates, and graph-state update symbols.  SNARK has proved that a
...
structure; the \texttt{MW-RUN-*} layer supplies the host-language semantics
...
paper, and the Isabelle theory page summarizing a Manna--Waldinger-style
unification formalization.
...
\item Add a separate new-example suite for stable merge, stable partition,
addition simplification, and shortest-path relaxation.
...
Loading the synthesis example file and running the executable demo function
produced:
\begin{lstlisting}
((UNIFICATION
