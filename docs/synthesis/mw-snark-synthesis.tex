...
\texttt{MW-SQUARE}, \texttt{MW-PLUS}, and \texttt{MW-SQRT}.  SNARK has proved
that a witness with this shape exists under the stated obligations; it has not,
by itself, supplied concrete Common Lisp definitions for those abstract symbols.
...
\texttt{(mw-const a)}.  The following command runs both executable
demonstrations:
\begin{lstlisting}
(mw-executable-synthesis-demos)
\end{lstlisting}
